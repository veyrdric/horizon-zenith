...
import { updateMaterialAction } from '@/lib/admin/actions';
...
export default function EditMaterialClient({ material }: { material: any }) {
  const router = useRouter();
  const [title, setTitle] = useState(material.title || '');
  const [tags, setTags] = useState<string[]>(material.tags || []);
  const [value, setValue] = useState(material.content || '');
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    const res = await updateMaterialAction(material.id, { title, content: value, tags });
    if (!res.error) {
      router.push(`/admin/library/${material.id}`);
    } else {
      alert(res.error);
      setIsSaving(false);
    }
    ...
    >
      ...
      <div className="py-4 min-h-[80px] border-b border-white/10 flex items-center justify-between px-6 bg-surface-dark shrink-0">
        <div className="flex items-center gap-4 flex-1">
          <Link href={`/admin/library/${material.id}`} className="p-2 rounded-xl bg-white/5 hover:bg-white/10 text-white transition-colors">
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => router.push(`/admin/library/${material.id}`)}
